...
actor_weights = rpyc_deep_copy(actor_weights)
critic_weights = rpyc_deep_copy(critic_weights)
for i in range(len(actor_weights)):
    actor_weights[i] = tf.io.parse_tensor(tf.constant(actor_weights[i]), out_type=tf.float32)
for i in range(len(critic_weights)):
    critic_weights[i] = tf.io.parse_tensor(tf.constant(critic_weights[i]), out_type=tf.float32)
