...
single["step"] = pd.cut(single["step"], bins=np.linspace(0, 6_00_000, 600), labels=np.linspace(0, 6_00_000, 599)+1000).astype(int)
single = single.groupby("step", as_index=False)["length"].agg(["mean", "std"]).reset_index().fillna(0)

dist = pd.concat([pd.read_csv("233.csv"), pd.read_csv("1262.csv"), pd.read_csv("2345.csv"), pd.read_csv("3707.csv"), pd.read_csv("2604.csv")])
...
fig, ax = plt.subplots()
fig.suptitle("Episode Length vs Training Steps in Lunar Lander Continuous")
...
plt.savefig("lunar_length.png", format="png")
plt.show()
